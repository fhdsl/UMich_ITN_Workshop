...
The `gct` file is a Gene Cluster Text file format used to describe an expression dataset. [See more about `gct` files at this Broad Institute resource.](https://software.broadinstitute.org/cancer/software/gsea/wiki/index.php/Data_formats#GCT:_Gene_Cluster_Text_file_format_.28.2A.gct.29)
...
<input type="checkbox"> Click on the produced file from the comparative marker selection.
...
An ODF is an Output Description Format file, a GenePattern specific filetype. It is similar to a GCT file format, describing a gene expression dataset; however, the ODF filetype has a specialized header. [See more information from GenePattern.](https://www.genepattern.org/file-formats-guide/#ODF)

```{r echo=FALSE, fig.alt = "The O D F file is a Gene Pattern specific file type that is similar to a G C T file type, but has a specialized header"}
...
<input type="checkbox"> In the opened side panel, under the "Send to Module" heading, click on "DifferentialExpressionViewer".
...
<input type="checkbox"> Select the `gct` file (`BRCA_HUGO_symbols.preprocessed.gct`) for the "dataset filename*" option.
