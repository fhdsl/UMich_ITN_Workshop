...
compatible with outputs fromthe CosMx-SMI spatial single-cell platform.
...
```{r install_spatialGE_secretly, eval=TRUE, echo = FALSE}
devtools::install_github("fridleylab/spatialGE", token = Sys.getenv('GH_PAT'))
```

```{r install_spatialGEshow, eval=FALSE}
...
```{r echo=FALSE}
library(here)
counts_fp = c(here("activity_files/bonus_spatial_transcriptomics_activity_files/Lung5_Rep1-Flat_files_and_images/Lung5_Rep1_exprMat_file_SUBSET.csv"),
              here("activity_files/bonus_spatial_transcriptomics_activity_files/Lung6-Flat_files_and_images/Lung6_exprMat_file_SUBSET.csv"))

coordinates_fp = c(here("activity_files/bonus_spatial_transcriptomics_activity_files/Lung5_Rep1-Flat_files_and_images/Lung5_Rep1_metadata_file_SUBSET.csv"),
                   here("activity_files/bonus_spatial_transcriptomics_activity_files/Lung6-Flat_files_and_images/Lung6_metadata_file_SUBSET.csv"))
```
...
Optinally, if tissue images are available for some or all the FOVs, users can
...
created at the beggining of this tutorial will be used to generate the file paths
to the folder containng the images. Importantly, the images need to be renamed
...
```{r loadimages1Run, echo=FALSE}
img_fp = here("activity_files/bonus_spatial_transcriptomics_activity_files/Lung5_Rep1-Flat_files_and_images/CellComposite")
lc_stlist <- load_images(lc_stlist, images=img_fp)
```
...
```{r loadimages2Run, echo=FALSE}
img_fp = here("activity_files/bonus_spatial_transcriptomics_activity_files/Lung6-Flat_files_and_images/CellComposite")
lc_stlist <- load_images(lc_stlist, images=img_fp)
```
...
Expression of KRT genes in domains 1 and 2 of FOVs "Lung6_fov_4" and "Lung6_fov_7", and
domains 2 and 4 of "Lung6_fov_6" are indicative of tumor cells. In those domains
(with the exception of 4 in "Lung6_fov_6"), there seems to be an overlap with the
areas of of NOTCH signaling, which indicates that in slide "Lung6", the tumor
compartment is spatially enriched for NOTCH signaling.
...
the column in `lung_subset@spatial_meta` containing the domain assigments:
...
The key columns to look ar are `min_spearman_r` and `min_spearman_r_pval_adj`.
Now, to find genes with significant spatial gradients across the three samples,
...
It can be seen from these results that multiple keratin genes show spatial gradients,
all of them with negative Spearman's coefficients (cells close to domain 3 tend to
have higher keratin expression). Other such __HLA-B__ and __NDRG1__ have higher
expression in cells far from domain 3:
...
Although the correlations in genes from this pathway are not as large as and those
for __KRT6A__ and __HLA-B__ (i.e., lower absolute Spearman's coefficient), it can
be seen that the genes __NOTCH1__ and __NOTCH3__ show evidence of expression
gradients in FOVs 'Lung6_fov_4' and 'Lung6_fov_6'.
