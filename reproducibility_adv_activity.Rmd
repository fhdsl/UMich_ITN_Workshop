...
<input type="checkbox"> Now if want to launch the image for use, we can use run button that looks like a triangle to turn the image into a **container**
...
<input type="checkbox"> In the `Exec` window on Docker Desktop, to run a bash script using the Docker container we could type bash and the name of the script and press enter. Copy paste the following for one the files from playground repository:
...
<input type="checkbox">  Alternatively, using the command line in terminal, now that we have the Docker container in execute mode, (where we see a `#` at the end of the line) we can enter the same command:
...
(We can type `exit` as a command to get back out of the execute mode for the container.)

Or we could do it all at once (but this would require not being in execute mode first):
...
This won't work because the file `run_analysis.sh` is not a file that our container has. Docker containers do not have all the files that our computer does; they only have the files we add to it. With all methods above, we will get this response:
...
```
docker run -v $PWD:/home jhudsl/reproducible-r -p 8787:8787 

```

For context, we previously ran this command to run the container without the files:
...
<input type="checkbox"> Click `Exec` to launch the execute mode.

<input type="checkbox"> Go to the home directory specified to place our files by typing the following and pressing enter after:

```
cd /home
```

<input type="checkbox"> Now try to run the script again.
...
<input type="checkbox"> Alternatively in the terminal we could also use the following:
```
